...
if __name__ == "__main__":   
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "vgg16_ela_best_model.pth")
            print(f"--> Best VGG16 model saved")
    ...
    plt.title("ELA VGG16: Training & Validation Loss", fontsize=14)
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.legend()
    ...
    plt.title("ELA VGG16: Validation Accuracy", fontsize=14)
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy (%)")
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6)
