...
if __name__ == "__main__":   
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "cnn128_ela_best_model.pth")
            print(f"--> Best ELA model saved at epoch {epoch}")
    ...
    plt.title("ELA CNN: Training & Validation Loss", fontsize=14)
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.legend()
    ...
    plt.title("ELA CNN: Validation Accuracy", fontsize=14)
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy (%)")
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6)
    ...
    plt.tight_layout()
    plt.savefig("cnn128_ela_plot.png")
    print("Το διάγραμμα αποθηκεύτηκε ως: cnn128_ela_plot.png")
    plt.show()